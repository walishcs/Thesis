...
\subsubsection{Absolute Possessive free}

Seediq also has a set of absolute possessive  free personal pronouns, which are typically used independently as the predicate or the ``subject''. Their usage is almost equivalent to English absolute possessive pronouns such as ``mine'', ``yours'', etc. The reflexes in the modern dialects are shown in Table \ref{tab:possfree}. Note that the abbriviation of absolute possesive used herein is \acs{abpos}.
