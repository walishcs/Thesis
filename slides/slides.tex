...
\begin{itemize}
    \item<1-> Stress: penultimate
    \item<2-> Word-final *-p, *-b, *-d, *-c, *-m, *-ag are not allowed
    \item<3-> Only one word starts with an *x-, which is *xiluy `iron'
\end{itemize}
\end{frame}
...
\begin{itemize}
    \item<1-> The case marking system of Proto-Seediq: *ka `\textsc{nom}' and *na `\textsc{gen}'
    \item<2-> Seediq case markers do not distinguish between noun categories
    \item<3-> Tgdaya preserves both, but other dialects loss *na `\textsc{gen}'
\end{itemize}
\end{frame}
...
\begin{itemize}
    \item<1-> There are 5 sets of personal pronouns can be reconstructed to \psed
    \begin{itemize}
        \item<2-> Nominative clitic
        \item<2-> Genitive clitic
        \item<2-> Nominative free
        \item<2-> Oblique free
        \item<2-> Absolute possessive free
    \end{itemize}
    \item<3-> There are also 3 portmanteau forms
    \item<4-> Each dialect has their own changes, some were borrowings
\end{itemize}
\end{frame}
...
\begin{itemize}
    \item<1-> Four voices: 
    \begin{itemize}
        \item<2-> Agent/Actor Voice (AV)
        \item<2-> Patient Voice (PV)
        \item<2-> Locative Voice (LV)
        \item<2-> Circumstancial Voice (CV)
    \end{itemize}
    \item<3-> Three sets:
    \begin{itemize}
        \item<4-> Set I: Realis
        \item<4-> Set II: Subjuctive (Negating current/past event; imperative)
        \item<4-> Set III: Hortative
    \end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
    \item<1-> Sometimes there are competing forms in different dialects
    \item<2-> When reconstructing \psed, I refer to evidence from \pan or \pata
    \item<3-> I prioritize evidence from \pan; if it is unavailable or significantly divergent, and if \pata has corresponding forms, then I adopt the evidence from \pata.
\end{itemize}
\end{frame}
...
\begin{frame}{Summary}
\begin{itemize}
    \item<1-> \textbf{\psed Phonology:} Reconstructed the sound system with 18 consonants, 4 monophthongs, 3 diphthongs, and phonotactic constraints.
    \item<2-> \textbf{\psed Morphosyntax:} Reconstructed nominal and verbal morphology, including case markers, pronominal, voice system, etc.
    \item<3-> \textbf{\psed Lexical Reconstruction:} Integrated Proto-Atayal and Proto-Austronesian evidence, analyzing lexical ``doublets'' and possible borrowings.
    \item<4-> \textbf{Dialect Subgrouping:} Proposed a new classification, identifying Tgdaya as the most divergent and grouping Toda-Truku based on shared innovations.
\end{itemize}
\end{frame}

\begin{frame}{Remaining issues - 1}
\begin{itemize}
    \item<1-> \textbf{Multiple Reflexes for \pan Phonemes:} Several \pan phonemes exhibit more than one reflex in \psed. The *q > *q/∅ split relates to historical layers, some splits may stem from phonetic or perceptual factors (*R > *g/r), the others splitfor unknown reasons, as also seen in Atayal (*C > *c/*s/*l).
    \item<2-> \textbf{Seediq-Atayal Relationship:} Evidence indicates significant borrowing from Atayal (loanwords and the phoneme *q) across dialects, calling for further comparison between Seediq and Atayal.
\end{itemize}
\end{frame}

\begin{frame}{Remaining issues - 2}
\begin{itemize}
    \item<1-> \textbf{Dialect Shifts Due to Relocations:} Some dialects (e.g., Tgdaya in Hualien, Paran community, Tawsay in Yilan) have shifted or lost features following forced relocations.
    \item<2-> \textbf{Unpublished data:} Accessing unpublished records and field notes (like Tsuchida's material) could enhance Proto-Seediq reconstruction and clarify finer subgrouping.
\end{itemize}
\end{frame}
